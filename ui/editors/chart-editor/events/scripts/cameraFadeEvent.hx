...
class CameraFadeEvent extends ScriptedSongEvent {
  ...
  **/
  ...
  public var shouldFixFadeEvent:Bool = true;
  public var camFadeVal = null;
  public var camHUDFadeVal = null;
  ...
  override function handleEvent(data) {
    ...
    if (!applyToHud) {
      ...
      FlxG.camera.fade(color, durSeconds / PlayState.instance.playbackRate, shouldFadeIn); // Fades camGame
    } else {
      PlayState.instance.camHUD?.stopFade();
      PlayState.instance.camHUD?.fade(color, durSeconds / PlayState.instance.playbackRate, shouldFadeIn); // camHUD (no point in having "toggle both" option since camHUD is above everything)
    }

    if (color == 11 || color == 0x00000000 && PlayState.instance.camHUD != null) {
      FlxTween.tween(PlayState.instance?.camHUD, {alpha: (shouldFadeIn ? 1.0 : 0.00001)}, durSeconds / PlayState.instance.playbackRate);
    }
  ...
  // Prevents the event from running if the game is paused
  // Credit goes to ComedyLost
  public override function onPause(e) {
    super.onPause(e);
    camFadeVal = PlayState.instance.camGame._fxFadeDuration ?? null;
    camHUDFadeVal = PlayState.instance.camHUD._fxFadeDuration ?? null;
    if (camFadeVal != null || camFadeVal != 0) PlayState.instance?.camGame?._fxFadeDuration = 0;
    if (camHUDFadeVal != null || camHUDFadeVal != 0) PlayState.instance?.camHUD?._fxFadeDuration = 0;
  }

  public override function onResume(e) {
    super.onResume(e);
    if (camFadeVal != null || camFadeVal != 0) PlayState.instance?.camGame?._fxFadeDuration = camFadeVal;
    if (camHUDFadeVal != null || camHUDFadeVal != 0) PlayState.instance?.camHUD?._fxFadeDuration = camHUDFadeVal;
  }
  ...
  override function getEventSchema(){
    return [
      ...
      {
        name: 'applyToHud',
        title: 'Layer it above camHUD\n(ignored when Transparent\nis picked\nas the color)',
        defaultValue: false,
        type: "bool"
      }
    ];
