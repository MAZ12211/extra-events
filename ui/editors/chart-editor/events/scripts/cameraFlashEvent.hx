...
class CameraFlashEvent extends ScriptedSongEvent {
  ...
  */
  ...
  public var shouldFixFlashEvent:Bool = true;
  ...
  override function handleEvent(data) {
    ...
    if (!applyToHud) {
      FlxG.camera.flash(color, durSeconds / PlayState.instance.playbackRate);
    } else {
      PlayState.instance.camHUD?.flash(color, durSeconds / PlayState.instance.playbackRate);
    }
