...
class CountDownEvent extends ScriptedSongEvent {
	...
	**/
	...
	override function handleEvent(data) {
		...
		if (PlayState.instance.isChartingMode) return; // TODO: song does not play in charting mode when countdown is disabled
...
class CountDownHandler extends Module {
	...
	function playCountdownSound(step:CountdownStep) {
		...
		countDownSound.pitch = PlayState.instance.playbackRate;
